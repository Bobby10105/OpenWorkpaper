...
export const generateAuditDocx = async (audit: AuditWithRelations): Promise<Blob> => {
  ...
  for (const phase of phases) {
    ...
    // Render Groups
    for (const [groupIndex, group] of Array.from(phaseGroups.entries())) {
      ...
      for (const [procIndex, p] of Array.from(sortedProcedures.entries())) {
    ...
    if (phaseUngrouped.length > 0) {
      ...
      for (const [index, p] of Array.from(phaseUngrouped.entries())) {
